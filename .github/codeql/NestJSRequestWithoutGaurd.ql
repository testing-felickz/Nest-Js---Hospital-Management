...
// - Allow for Controllers without Gaurd if all endpoints have a Gaurd applied (also loop through MethodDefinitions not just ClassDefinitions)
// - Allow for a list of known endpoints where guards that are not required (consider using data extensions)
// - Detect use of global scoped gaurds `app.useGlobalGuards` (dependency injection cases)
// - Detect Gaurds that do not apply authorization (heurestics by name or by allowlist in data extensions for strict enforcement)
